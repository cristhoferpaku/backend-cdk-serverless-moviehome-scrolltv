...
import { CreateMultimediaCategoriesService } from './services/createMultimediaCategory.service';
import { CreateMultimediaCategoriesRequest } from './dtos/createMultimediaCategory.dto';
import {
...
const FUNCTION_NAME = 'CreateMultimediaCategoriesHandler';
...
export const handler = async (
  ...
  try {
    ...
    const body = parseRequestBody<CreateMultimediaCategoriesRequest>(event.body);
    if (!body) {
    ...
    const service = new CreateMultimediaCategoriesService();
    const result = await service.createMultimediaCategories(body);
