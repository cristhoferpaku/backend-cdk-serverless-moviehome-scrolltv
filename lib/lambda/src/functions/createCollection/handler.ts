...
import { CreateCollectionsService } from './services/createCollection.service';
import { CreateCollectionsRequest } from './dtos/createCollection.dto';
import {
...
const FUNCTION_NAME = 'CreateCollectionsHandler';
...
export const handler = async (
  ...
  try {
    ...
    const body = parseRequestBody<CreateCollectionsRequest>(event.body);
    if (!body) {
    ...
    const service = new CreateCollectionsService();
    const result = await service.createCollections(body);
