...
import { UpdateEpisodeService } from './services/updateEpisode.service';
import {
...
import { UpdateEpisodeRequest } from './dtos/updateEpisode.dto';

const FUNCTION_NAME = 'UpdateEpisodeHandler';
...
export const handler = async (
  ...
  try {
    ...
    const body = parseRequestBody<UpdateEpisodeRequest>(event.body);
    if (!body || Object.keys(body).length === 0) {
      return createBadRequestResponse('Datos de actualización requeridos', event);
    }

    const service = new UpdateEpisodeService();
    const result = await service.updateEpisode(id, body);
