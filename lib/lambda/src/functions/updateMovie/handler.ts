...
import { UpdateMovieRequest } from './dtos/updateMovie.dto';

const FUNCTION_NAME = 'UpdateMovieHandler';
...
export const handler = async (
  ...
  try {
    ...
    const body = parseRequestBody<UpdateMovieRequest>(event.body);
...
//];
  ...
  //  if (validationErrors.length > 0) {
   ...
   // }
    ...
    const result = await service.updateMovie(id, body);
