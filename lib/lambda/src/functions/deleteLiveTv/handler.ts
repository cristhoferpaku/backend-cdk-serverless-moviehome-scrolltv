...
import { DeleteLiveTvService } from './services/deleteLiveTv.service';
import {
  createOkResponse,
...
const FUNCTION_NAME = 'DeleteLiveTvHandler';
...
export const handler = async (
  ...
  try {
    ...
    if (!idParam || isNaN(id) || id <= 0) {
      return createBadRequestResponse('El parámetro {id} del LiveTV es requerido y debe ser un número válido', event);
    }

    const service = new DeleteLiveTvService();
    const result = await service.deleteLiveTv({ id });
