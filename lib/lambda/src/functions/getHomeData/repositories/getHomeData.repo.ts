...
export class GetHomeRepository {
   ...
   */
  async getAllHome(sectionId: number, user_id : number): Promise<HomeData> {
    const query = `
      SELECT * FROM sp_get_home_data($1, $2);
    `;

   const { rows } = await dbConnector.query(query, [sectionId, user_id]);
