...
import { ChangeCollectionsStatusService } from './services/changeCollectionStatus.service';
import { ChangeCollectionsStatusBody, ChangeCollectionsStatusRequestDto } from './dtos/changeCollectionStatus.dto';
import { 
...
const FUNCTION_NAME = 'ChangeCollectionsStatusHandler';
...
export const handler = async (
  ...
  try {
    ...
    // Validar parámetro CollectionsId
   ...
   const request: ChangeCollectionsStatusRequestDto ={
      id: Number(idParam),
      status: body.status
      
    };
    // Ejecutar servicio
    const service = new ChangeCollectionsStatusService();
    const result = await service.changeCollectionsStatus(request);
