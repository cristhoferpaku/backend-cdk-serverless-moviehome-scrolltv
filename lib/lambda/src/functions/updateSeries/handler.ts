...
import { UpdateSerieService } from './services/updateSeries.service';
import {
...
import { UpdateSerieRequest } from './dtos/updateSeries.dto';

const FUNCTION_NAME = 'UpdateSerieHandler';
...
export const handler = async (
  ...
  try {
    ...
    const body = parseRequestBody<UpdateSerieRequest>(event.body);
...
//];
  ...
  //  if (validationErrors.length > 0) {
   ...
   // }
    const service = new UpdateSerieService();
    const result = await service.updateSerie(id, body);
