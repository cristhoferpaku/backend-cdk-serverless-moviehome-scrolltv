...
import { ChangeLiveTvStateService } from './services/changeLiveTvStatus.service';
import { ChangeLiveTvStateBody, ChangeLiveTvStateRequestDto } from './dtos/changeLiveTvStatus.dto';
import { 
...
const FUNCTION_NAME = 'ChangeLiveTvStateHandler';
...
export const handler = async (
  ...
  try {
   ...
   const request: ChangeLiveTvStateRequestDto ={
      id: Number(idParam),
      status: body.status
      
    };
    // Ejecutar servicio
    const service = new ChangeLiveTvStateService();
    const result = await service.changeLiveTvState(request);
